...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configAuthentication(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.jdbcAuthentication().dataSource(dataSource)
                .usersByUsernameQuery("select name as username,password, enabled from APP_USER where name=?")
                .authoritiesByUsernameQuery("SELECT APP_USER.name as username, APP_ROLE.name as roles FROM APP_USER,APP_ROLE where APP_USER.name= ?");
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/#/administrator/**","/administrator/**").hasAnyRole("ADMIN")
